...
class DHCPManager {
    ...
    showAddReservationDialog() {
        const ip = prompt('请输入要保留的IP地址:');
        if (!ip) return;

        const mac = prompt('请输入MAC地址:');
        if (!mac) return;

        const hostname = prompt('请输入主机名 (可选):') || '';

        this.addReservation(ip, mac, hostname);
    }

    async addReservation(ip, mac, hostname) {
        try {
            const response = await this.authManager.fetchWithAuth('/api/dhcp/reservations', {
                method: 'POST',
                headers: {
            ...
            if (response.ok) {
                await this.loadDHCPData();
                this.showMessage('IP保留添加成功', 'success');
            } else {
                const error = await response.text();
                this.showMessage(`添加保留失败: ${error}`, 'error');
            }
        } catch (error) {
            console.error('添加IP保留出错:', error);
            this.showMessage('添加保留失败', 'error');
        }
    }

    editReservation(ip, mac, hostname) {
        const newMac = prompt('请输入新的MAC地址:', mac);
        if (!newMac) return;

        const newHostname = prompt('请输入新的主机名:', hostname) || '';

        this.updateReservation(ip, newMac, newHostname);
    }

    async updateReservation(ip, mac, hostname) {
        try {
            const response = await this.authManager.fetchWithAuth('/api/dhcp/reservations', {
                method: 'PUT',
                headers: {
            ...
            if (response.ok) {
                await this.loadDHCPData();
                this.showMessage('IP保留更新成功', 'success');
            } else {
                const error = await response.text();
                this.showMessage(`更新保留失败: ${error}`, 'error');
            }
        } catch (error) {
